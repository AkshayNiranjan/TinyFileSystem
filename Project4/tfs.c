...
int get_avail_ino() {
	...
	bitmap_t ibitmap = (bitmap_t) calloc(1,4096);
...
int get_avail_blkno() {
	...
	bitmap_t dbitmap = (bitmap_t) calloc(1,4096);
...
int get_ind_ptr_loc(struct inode *in,int blockno){
	int *indirect_ptr=in->indirect_ptr;
	uint32_t* ind_block=malloc(4096);
	for(int i=0;i<8;i++){
		if(indirect_ptr[i]==0){
			...
			memset(ind_block,0,4096);
			ind_block[0]=blockno;
			bio_write(in->ino+sb.i_start_blk,ind_block);
			free(ind_block);
			return i*1024;
		}
		bio_read(indirect_ptr[i]+sb.d_start_blk,ind_block);
...
struct dirent * getFnameDirent(struct inode dirinode, const char *fname) {
	...
	for(i = 0; i < dirinode.link && i<16; i++) {
	 	bio_read(dirinode.direct_ptr[i], datablock);
	...
	int * inddatablock = calloc(1,4096);
	for(i = 0; (i*1025)+16 < dirinode.link; i++) {
	 	bio_read(dirinode.indirect_ptr[i],(void*) inddatablock);

		for(int j=0;j<1025 && j+(i*1025)+17<dirinode.link;j++){
			bio_read(inddatablock[j],datablock);
			datablockdirent = (struct dirent *) datablock;
			while((void *) datablockdirent <= datablock + 4096 - sizeof(struct dirent)){
				if(datablockdirent->valid == 1 && !strcmp(fname, datablockdirent->name)){
					free(datablock);
 ...
 */
 int help_ind_read(int blockno,int index_ind){
 	int *datablock = calloc(1,4096);
 	if(bio_read(blockno, (void*)datablock)!=0)
 		return -1;
 	return datablock[index_ind];
 }
 int help_ind_write(int blockno,int newblock,int index_ind){
 	int *datablock = calloc(1,4096);
 	if(bio_read(blockno, (void*)datablock)!=0)
 		return -1;
 	datablock[index_ind]=newblock;
 	return bio_write(blockno,(void*)datablock);
 }
...
int dir_add(struct inode dir_inode, uint16_t f_ino, const char *fname, size_t name_len) {
	...
	if(datablockcount < 16) {
		...
		void * newdatablock = calloc(1,4096);
	...
	//Dirinode full
		...
		if(datablockcount < 8216) {//16 for direct 8 for indirect each has 1024 datablocks
			//printf(", adding datablock: %d\n", datablockcount);
			//Get free block
			int blockno = get_avail_blkno();
			if(blockno < 0) {
			...
			//update parent dir inode
			int index_ind=(datablockcount-16)/1024;
			if((datablockcount-16)%1025==0){
				dir_inode.indirect_ptr[index_ind] = blockno;
				dir_inode.link = (uint32_t) datablockcount + 2;
				writei(dir_inode.ino, &dir_inode);

				blockno = get_avail_blkno();
				if(blockno < 0) {
					return blockno;
				}
				blockno += sb.d_start_blk;
				uint32_t * newdatablock = malloc(4096);
				newdatablock[0]=blockno;
				bio_write(dir_inode.indirect_ptr[index_ind],newdatablock);
				free(newdatablock);
				//add data block with new dirent
				newdatablock = calloc(1,4096);
				struct dirent * newdirent = (struct dirent *) newdatablock;
				newdirent->valid = 1;
				newdirent->ino = f_ino;
				memset(newdirent->name, '\0', 252);
			...
			int* allblocks=malloc(4096);
			bio_read(dir_inode.indirect_ptr[index_ind],allblocks);
			int currblock=allblocks[(datablockcount-16)%1025];
			allblocks[currblock+1]=blockno;
			bio_write(dir_inode.indirect_ptr[index_ind],allblocks);
			free(allblocks);
			//dir_inode.indirect_ptr[(datablockcount-1)/1024] = blockno;
			dir_inode.link = (uint32_t) datablockcount + 1;
			writei(dir_inode.ino, &dir_inode);
...
int dir_remove(struct inode dir_inode, const char *fname, size_t name_len) {
	...
	int i;
	struct dirent * datablockdirent;
	for(i = 0; i < dir_inode.link && i<16; i++) {
	 	bio_read(dir_inode.direct_ptr[i], datablock);
...
char * getParentDir(const char * path) {
	...
	if(!strcmp(path, forwardslash)){
		char * forwardslashpath = (char *) callos(1,2);
		forwardslashpath[0] = '/';
	...
	//printf("Index: %d\n", index);
	char * parentDirString = (char *) calloc(1,index);
...
static int tfs_rmdir(const char *path) {
	...
	if(found < 0) {
		...
		//insert parse through indirect methods
		bio_write(sb.d_bitmap_blk, dbitmap);
